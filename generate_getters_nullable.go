...
func (g *codeGenerator) createGetterSetterUint64Nullable(schema *entitySchema, fieldName, entityName, getterSuffix, providerName string) {
	if getterSuffix != "" {
		...
		g.addLine("\t\t\t\t\treturn uint64(vNullable.Int64)")
		g.addLine("\t\t\t\t}")
		g.addLine("\t\t\t\treturn 0")
		g.addLine("\t\t\t}")
		g.addLine("\t\t}")
		...
		g.addLine(fmt.Sprintf("\t\treturn uint64(e.originDatabaseValues.F%d.Int64)", g.filedIndex))
		g.addLine("\t}")
		g.addLine("\treturn 0")
		g.addLine("}")
	} else {
...
func (g *codeGenerator) createGetterSetterBytesNullable(schema *entitySchema, fieldName, entityName, providerName string) {
	...
	g.addLine(fmt.Sprintf("func (e *%s) Set%s(value []uint8) *%s {", entityName, fieldName, entityName))
	g.addLine("\tbindValue := sql.NullString{}")
	g.addLine("\tif value != nil {")
	g.addLine("\t\tbindValue.Valid = true")
...
func (g *codeGenerator) createGetterSetterJsonStruct(schema *entitySchema, fieldName, entityName, providerName, goTypeName string) {
	...
	g.addLine(fmt.Sprintf("func (e *%s) Set%s(value *%s) *%s {", entityName, fieldName, goTypeName, entityName))
	g.addLine("\tbindValue := sql.NullString{}")
	g.addLine("\tif value != nil {")
