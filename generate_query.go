...
func (g *codeGenerator) generateGetByID(schema *entitySchema, names *entityNames) {
	...
	g.appendToLine(fmt.Sprintf("\tfound, err = ctx.DB(%s.dbCode).QueryRow(ctx, fluxaorm.NewWhere(query, id), &sqlRow.F0", names.providerName))
...
func (g *codeGenerator) generateGetByIDs(schema *entitySchema, names *entityNames) {
	...
	if schema.hasRedisCache {
		g.addLine("\t\t_useCache := !ctx.InTransaction()")
		g.addLine("\t\tmissingIDs := idsToFetch")
		g.addLine("\t\tif _useCache {")
		...
		g.addLine("\t\t\t}")
		g.addLine("\t\t\tb.WriteByte(')')")
		g.addLine(fmt.Sprintf("\t\t\trows, cl, err := ctx.DB(%s.dbCode).Query(ctx, b.String())", names.providerName))
		g.addLine("\t\t\tif err != nil {")
		g.addLine("\t\t\t\treturn nil, err")
		g.addLine("\t\t\t}")
		g.addLine("\t\t\tdefer cl()")
		...
		g.addLine("\t\t\t\tif _useCache {")
		g.addLine("\t\t\t\t\t_fillKey := p.redisCachePrefix + strconv.FormatUint(sqlRow.F0, 10)")
		g.addLine("\t\t\t\t\tcachePipeline.Del(_fillKey)")
		g.addLine("\t\t\t\t\tcachePipeline.RPush(_fillKey, sqlRow.redisValues()...)")
		g.addLine("\t\t\t\t\tcachePipeline.Expire(_fillKey, fluxaorm.EntityCacheTTL)")
		g.addLine("\t\t\t\t}")
		...
		g.addLine("\t\t\t\t\t\tcachePipeline.Expire(redisKey, fluxaorm.EntityCacheTTL)")
		g.addLine("\t\t\t\t\t}")
		g.addLine("\t\t\t\t}")
		g.addLine("\t\t\t}")
