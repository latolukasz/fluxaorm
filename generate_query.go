...
func (g *codeGenerator) generateGetByID(schema *entitySchema, names *entityNames) {
	...
	if schema.hasRedisCache {
		...
		g.addLine(fmt.Sprintf("\t\treturn &%s{ctx: ctx, id: id, originRedisValues: redisValues[1:]}, true, nil", names.entityName))
		g.addLine("\t}")
	}
	...
	g.addLine(fmt.Sprintf("\treturn &%s{ctx: ctx, id: id, originDatabaseValues: sqlRow}, true, nil", names.entityName))
	g.addLine("}")
	g.addLine("")
}
...
func (g *codeGenerator) generateGetByIDs(schema *entitySchema, names *entityNames) {
	...
	if schema.hasRedisCache {
		...
		g.addLine(fmt.Sprintf("\t\t\tresultMap[v] = &%s{ctx: ctx, id: v, originRedisValues: values[1:]}", names.entityName))
		g.addLine("\t\t\tcontinue")
		g.addLine("\t\t}")
		...
		g.addLine("\t\t\t\treturn nil, err")
		g.addLine("\t\t\t}")
		g.addLine(fmt.Sprintf("\t\t\tresultMap[sqlRow.F0] = &%s{ctx: ctx, id: sqlRow.F0, originDatabaseValues: sqlRow}", names.entityName))
