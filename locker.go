...
func (l *Locker) fillMetrics(ctx Context, end time.Duration, set, miss bool, err error) {
	...
	if hasMetrics {
		...
		metrics.queriesRedis.WithLabelValues(metricsOperationLock, l.r.config.GetCode(), setValue, missValue, "0", ctx.getMetricsSourceTag()).Observe(end.Seconds())
		if err != nil {
			metrics.queriesRedisErrors.WithLabelValues(l.r.config.GetCode(), ctx.getMetricsSourceTag()).Inc()
		}
	}
}
...
func (l *Locker) fillLogFields(ctx Context, operation, query string, duration time.Duration, cacheMiss bool, err error) {
	_, loggers := ctx.getRedisLoggers()
	fillLogFields(ctx, loggers, l.r.config.GetCode(), sourceRedis, operation, query, &duration, cacheMiss, err)
}
