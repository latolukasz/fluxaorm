...
func (g *codeGenerator) generateCodeForEntity(schema *entitySchema) error {
	...
	g.addLine(fmt.Sprintf("func (p %s) GetByIDs(ctx fluxaorm.Context, id ...uint64) ([]*%s, error) {", providerNamePrivate, entityName))
	g.addLine("\tif len(id) == 0 {")
	g.addLine("\t\treturn nil, nil")
	g.addLine("\t}")
	g.addLine(fmt.Sprintf("\tresultMap := make(map[uint64]*%s, len(id))", entityName))
	...
	if schema.hasRedisCache {
		g.addLine("\tredisPipeline := ctx.RedisPipeLine(p.redisCode)")
		g.addLine("\tredisResults := make([]*fluxaorm.PipeLineSlice, len(uniqueIDs))")
		g.addLine("\tfor i, v := range uniqueIDs {")
		...
		g.addLine("\t}")
		g.addLine("\tmissingIDs := make([]uint64, 0, len(uniqueIDs))")
		g.addLine("\tfor i, v := range uniqueIDs {")
		...
		g.addLine(fmt.Sprintf("\t\t\tresultMap[v] = &%s{ctx: ctx, id: v, originRedisValues: values[1:]}", entityName))
		g.addLine("\t\t\tcontinue")
		g.addLine("\t\t}")
		...
		g.addLine("\t\t}")
		g.addLine("\t\tb.WriteByte(')')")
		g.addLine(fmt.Sprintf("\t\trows, cl, err := ctx.Engine().DB(%s.dbCode).Query(ctx, b.String())", providerName))
		g.addLine("\t\tif err != nil {")
		g.addLine("\t\t\treturn nil, err")
		g.addLine("\t\t}")
		g.addLine("\t\tdefer cl()")
		g.addLine(fmt.Sprintf("\t\tfoundInDB := make(map[uint64]bool, len(missingIDs))"))
		...
		g.addLine("\t\t\t\treturn nil, err")
		g.addLine("\t\t\t}")
		g.addLine(fmt.Sprintf("\t\t\tresultMap[sqlRow.F0] = &%s{ctx: ctx, id: sqlRow.F0, originDatabaseValues: sqlRow}", entityName))
		g.addLine("\t\t\tfoundInDB[sqlRow.F0] = true")
		g.addLine(fmt.Sprintf("\t\t\tcachePipeline.RPush(p.redisCachePrefix+strconv.FormatUint(sqlRow.F0, 10), sqlRow.redisValues()...)"))
		g.addLine("\t\t}")
		g.addLine("\t\tfor _, v := range missingIDs {")
		g.addLine("\t\t\tif !foundInDB[v] {")
		g.addLine(fmt.Sprintf("\t\t\t\tredisKey := p.redisCachePrefix + strconv.FormatUint(v, 10)"))
		...
		g.addLine("\tb.WriteString(strconv.FormatUint(uniqueIDs[0], 10))")
		g.addLine("\tfor _, v := range uniqueIDs[1:] {")
		g.addLine("\t\tb.WriteByte(',')")
		g.addLine("\t\tb.WriteString(strconv.FormatUint(v, 10))")
		g.addLine("\t}")
		g.addLine("\tb.WriteByte(')')")
	...
	g.addLine(fmt.Sprintf("\tresult := make([]*%s, 0, len(uniqueIDs))", entityName))
	...
	g.addLine(fmt.Sprintf("\t_, err = ctx.Engine().DB(%s.dbCode).QueryRow(ctx, fluxaorm.NewWhere(b.String(), params...), &totalRows)", providerName))
	g.addLine("\tif err != nil {")
	...
	g.addLine("\t}")
	g.addLine("\tb.Reset()")
	selectPrefixSearchWithCount := "\"SELECT `ID`"
	for _, columnName := range schema.GetColumns()[1:] {
		selectPrefixSearchWithCount += ",`" + columnName + "`"
	}
	selectPrefixSearchWithCount += fmt.Sprintf(" FROM `%s`\"", schema.tableName)
	g.addLine(fmt.Sprintf("\tb.WriteString(%s)", selectPrefixSearchWithCount))
	g.addLine("\tb.WriteString(whereClause)")
	g.addLine("\tif pager != nil {")
	g.addLine("\t\tb.WriteByte(' ')")
	g.addLine("\t\tb.WriteString(pager.String())")
	g.addLine("\t}")
	g.addLine(fmt.Sprintf("\trows, cl, err := ctx.Engine().DB(%s.dbCode).Query(ctx, b.String(), params...)", providerName))
	g.addLine("\tif err != nil {")
	g.addLine("\t\treturn nil, 0, err")
	g.addLine("\t}")
	g.addLine("\tdefer cl()")
	...
	g.addLine("\t\t\treturn nil, 0, err")
	g.addLine("\t\t}")
	g.addLine(fmt.Sprintf("\t\tentities = append(entities, &%s{ctx: ctx, id: sqlRow.F0, originDatabaseValues: sqlRow})", entityName))
	g.addLine("\t}")
	...
	g.addImport("strings")
	g.addLine("\tvar b strings.Builder")
	selectPrefixSearch := "\"SELECT `ID`"
	for _, columnName := range schema.GetColumns()[1:] {
		selectPrefixSearch += ",`" + columnName + "`"
	}
	selectPrefixSearch += fmt.Sprintf(" FROM `%s`\"", schema.tableName)
	g.addLine(fmt.Sprintf("\tb.WriteString(%s)", selectPrefixSearch))
	g.addLine("\tvar params []any")
	if schema.hasFakeDelete {
