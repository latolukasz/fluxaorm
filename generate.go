...
func (g *codeGenerator) generateCodeForEntity(schema *entitySchema) error {
	...
	g.addImport("strconv")
	g.addLine(fmt.Sprintf(" FROM `%s` WHERE `ID` IN (\" + strconv.FormatUint(id[0], 10)", schema.tableName))
	g.addLine("\tfor _, value := range id[1:] {")
	g.addLine("\t\tquery += \",\" + strconv.FormatUint(value, 10)")
	g.addLine("\t}")
	g.addLine("\tquery += \")\"")
	g.addLine(fmt.Sprintf("\trows, cl, err := ctx.Engine().DB(%s.dbCode).Query(ctx, query)", providerName))
	g.addLine("\tif err != nil {")
	g.addLine("\t\treturn nil, err")
	g.addLine("\t}")
	g.addLine("\tdefer cl()")
	g.addLine(fmt.Sprintf("\tresult := make([]*%s, 0, len(id))", entityName))
	...
	g.addLine("\t\t\treturn nil, err")
	g.addLine("\t\t}")
	g.addLine(fmt.Sprintf("\t\tresult = append(result, &%s{ctx: ctx, id: sqlRow.F0, originDatabaseValues: sqlRow})", entityName))
	g.addLine("\t}")
