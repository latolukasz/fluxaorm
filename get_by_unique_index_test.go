...
var getByUniqueIndexEntityIndexes = struct {
	Name  UniqueIndexDefinition
	Multi UniqueIndexDefinition
	Ref   UniqueIndexDefinition
	Time  UniqueIndexDefinition
	Died  UniqueIndexDefinition
	Price UniqueIndexDefinition
}{
	Name:  UniqueIndexDefinition{"Name", true},
	Multi: UniqueIndexDefinition{"Age,Active", true},
	Ref:   UniqueIndexDefinition{"Ref", false},
	Time:  UniqueIndexDefinition{"BirthDate", false},
	Died:  UniqueIndexDefinition{"Died,DeathDate", false},
	Price: UniqueIndexDefinition{"Price", false},
}
...
type getByUniqueIndexEntity struct {
	...
	Age       uint8
	Active    bool
	Ref       Reference[getByUniqueIndexReference]
...
func (e *getByUniqueIndexEntity) Indexes() any {
	return getByUniqueIndexEntityIndexes
}
...
func testGetByUniqueIndex(t *testing.T, local, redis bool) {
	...
	entity, found, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Name, "Missing")
	assert.NoError(t, err)
	assert.False(t, found)
	assert.Nil(t, entity)

	entity, _, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Name, time.Now())
	assert.EqualError(t, err, "[Name] invalid value")

	entity, found, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Multi, 4, false)
	assert.NoError(t, err)
	assert.True(t, found)
	assert.NotNil(t, entity)
	...
	entity, found, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Multi, 4, 0)
	assert.NoError(t, err)
	assert.True(t, found)
	assert.NotNil(t, entity)
	assert.Equal(t, entities[4].ID, entity.ID)
	...
	for _, number := range numbers {
		entity, _, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Multi, number, 0)
		assert.NoError(t, err)
		assert.Equal(t, "Name 4", entity.Name)
	}
	...
	entity, _, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Multi, "invalid", 0)
	assert.EqualError(t, err, "[Age] invalid number invalid")

	entity, _, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Multi, time.Now(), 0)
	assert.EqualError(t, err, "[Age] invalid value")

	entity, found, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Ref, refs[4].ID)
	assert.NoError(t, err)
	assert.True(t, found)
	assert.NotNil(t, entity)
	assert.Equal(t, entities[4].ID, entity.ID)
	...
	entity, found, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Time, date)
	assert.NoError(t, err)
	assert.True(t, found)
	assert.NotNil(t, entity)
	...
	entity, found, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Died, true, died)
	assert.NoError(t, err)
	assert.True(t, found)
	assert.NotNil(t, entity)
	assert.Equal(t, entities[6].ID, entity.ID)
	...
	entity, found, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Died, 1, died)
	assert.NoError(t, err)
	assert.True(t, found)
	assert.NotNil(t, entity)
	assert.Equal(t, entities[6].ID, entity.ID)
	...
	died = died.Add(time.Hour * -72)
	entity, found, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Died, "false", died)
	assert.NoError(t, err)
	assert.True(t, found)
	assert.NotNil(t, entity)
	assert.Equal(t, entities[3].ID, entity.ID)
	...
	_, _, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Name, "a", "b")
	assert.EqualError(t, err, "invalid number of index `Name` attributes, got 2, 1 expected")

	_, _, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, UniqueIndexDefinition{"Invalid", false})
	...
	_, _, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Died, time.Now(), died)
	...
	err = r.FlushAll(orm)
	assert.NoError(t, err)
	_, found, err = GetByUniqueIndex[getByUniqueIndexEntity](orm, getByUniqueIndexEntityIndexes.Name, "Name 9")
	assert.NoError(t, err)
	assert.True(t, found)
}
